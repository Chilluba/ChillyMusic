...
import { usePlayback, RepeatMode } from '../context/PlaybackContext'; // Import RepeatMode
...
const PlayerScreen: React.FC = () => {
  ...
  useEffect(() => {
    const checkFavStatus = async () => {
      if (playback.currentTrack) {
        const favStatus = await checkIsFavorite(playback.currentTrack.videoId);
        setIsCurrentTrackFavorite(favStatus);
      } else {
  ...
  const handleToggleFavorite = async () => { /* ... existing ... */
    if (!playback.currentTrack) return;
    const trackDetails: Pick<FavoriteItem, 'videoId' | 'title' | 'channel' | 'thumbnail'> = {
      videoId: playback.currentTrack.videoId, title: playback.currentTrack.title,
      channel: 'channel' in playback.currentTrack ? playback.currentTrack.channel : ((playback.currentTrack as any).artist || undefined),
      thumbnail: playback.currentTrack.thumbnail
    };
    if (isCurrentTrackFavorite) {
      await removeFavorite(trackDetails.videoId); setIsCurrentTrackFavorite(false);
      Alert.alert('Unfavorited', `"${trackDetails.title}" removed from favorites.`);
    } else {
      await addFavorite(trackDetails); setIsCurrentTrackFavorite(true);
      Alert.alert('Favorited', `"${trackDetails.title}" added to favorites.`);
    }
  };
  ...
  const openDownloadOptionsForCurrentTrack = async () => { /* ... existing ... */
    if (!playback.currentTrack) { Alert.alert('Error', 'No track selected to download.'); return; }
    setIsDownloadModalVisible(true);
    if (mediaInfoForDownloadOpts?.videoId === playback.currentTrack.videoId) { setIsLoadingDownloadOptions(false); return; }
    setIsLoadingDownloadOptions(true);
    try {
      const info = await fetchMediaInfo(playback.currentTrack.videoId); setMediaInfoForDownloadOpts(info);
    } catch (error: any) { Alert.alert('Error', 'Could not fetch download options.'); setMediaInfoForDownloadOpts(null);
    } finally { setIsLoadingDownloadOptions(false); }
  };
  ...
  const styles = StyleSheet.create({ /* ... existing styles ... */
    ...
    actionButtonSmall: { padding: theme.spacing.sm, alignItems: 'center', minWidth: 60 },
  ...
  if (!currentTrack) { return <View style={styles.centered}><Text style={styles.emptyText}>No track loaded.</Text></View>; }
  ...
  let currentTrackDownloadDisplay: React.ReactNode = null; // ... (download display logic)
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.trackInfoContainer}><Text style={styles.title} numberOfLines={2}>{displayTitle}</Text><Text style={styles.artist} numberOfLines={1}>{displayArtist}</Text>{currentTrackPlaybackError && <Text style={[styles.errorText, {fontSize: theme.typography.fontSize.caption}]}>Error: {playback.error}</Text>}</View>
      <View style={styles.sliderContainer}><Slider style={styles.sliderComponent} minimumValue={0} maximumValue={trackDuration > 0 ? trackDuration : 1} value={currentTime} onSlidingComplete={(value) => playback.seekTo(value)} minimumTrackTintColor={theme.colors.accentPrimary} maximumTrackTintColor={theme.colors.textMuted} thumbTintColor={theme.colors.accentPrimary} disabled={playback.isLoading || trackDuration === 0 || !!playback.error} /><View style={styles.timeLabelsContainer}><Text style={styles.timeLabel}>{formatTime(currentTime)}</Text><Text style={styles.timeLabel}>{formatTime(trackDuration)}</Text></View></View>
      <View style={styles.controlsContainer}><TouchableOpacity onPress={() => {}} style={styles.controlButton} disabled={true}><Icon name="SkipBack" size={32} color={theme.colors.textMuted} /></TouchableOpacity><TouchableOpacity onPress={playback.togglePlayPause} style={[styles.controlButton, styles.playPauseButton, (playback.isLoading || !currentTrack || trackDuration === 0 || !!playback.error) && {backgroundColor: theme.colors.textMuted}]} disabled={playback.isLoading || !currentTrack || trackDuration === 0 || !!playback.error}>{(playback.isLoading && currentTrack) ? <ActivityIndicator size='large' color={theme.colors.white} /> : <Icon name={playback.isPlaying ? 'Pause' : 'Play'} size={40} color={theme.colors.white} />}</TouchableOpacity><TouchableOpacity onPress={() => {}} style={styles.controlButton} disabled={true}><Icon name="SkipForward" size={32} color={theme.colors.textMuted} /></TouchableOpacity></View>

      <View style={styles.actionsRow}>
        <TouchableOpacity style={styles.actionButtonSmall} onPress={playback.setRepeatMode}>
          <Icon name={playback.repeatMode === 'one' ? 'RepeatOne' : 'Repeat'} size={24} color={playback.repeatMode !== 'off' ? theme.colors.accentPrimary : theme.colors.textSecondary} />
        </TouchableOpacity>
        {/* ... Download Button ... */}
        <View style={styles.actionButtonSmall}><TouchableOpacity onPress={openDownloadOptionsForCurrentTrack} disabled={!currentTrack || isLoadingDownloadOptions || (!!currentTrackDownloadDisplay && currentTrackDownloadStatus !== 'Error' && currentTrackDownloadStatus !== 'Downloaded' )}><Icon name="Download" size={24} color={theme.colors.textSecondary} /></TouchableOpacity></View>
        <TouchableOpacity style={styles.actionButtonSmall} onPress={handleToggleFavorite} disabled={!currentTrack}>
          <Icon name={isCurrentTrackFavorite ? 'Heart' : 'HeartOutline'} size={24} color={isCurrentTrackFavorite ? theme.colors.accentPrimary : theme.colors.textSecondary} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionButtonSmall} onPress={() => Alert.alert('Add to Playlist', 'Playlist TBD')}><Icon name="PlaylistAdd" size={24} color={theme.colors.textSecondary} /></TouchableOpacity>
      </View>
      <DownloadOptionsModal visible={isDownloadModalVisible} mediaInfo={mediaInfoForDownloadOpts} isLoading={isLoadingDownloadOptions} onClose={() => { setIsDownloadModalVisible(false); setMediaInfoForDownloadOpts(null); }} onSelectOption={handleSelectDownloadOption}/>
    </ScrollView>
  );
};
