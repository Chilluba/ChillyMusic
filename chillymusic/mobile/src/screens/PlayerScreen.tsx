...
// import { useDownload } from '../context/DownloadContext'; // For next step

const PlayerScreen: React.FC = () => {
  const { theme } = useAppTheme();
  const playback = usePlayback();
  // const downloadCtrl = useDownload(); // For next step
  ...
  const openDownloadOptionsForCurrentTrack = async () => {
    ...
    // If mediaInfo for this track is already in context (e.g. if PlaybackContext fetched it),
    // we could potentially use it. However, DownloadOptionsModal might want freshest/all formats.
    // For now, always fetch for the modal unless it's already cached specifically for download options.
    if (mediaInfoForDownloadOpts?.videoId === playback.currentTrack.videoId) {
        setIsLoadingDownloadOptions(false); // Already have it for this track
        return;
    }
    ...
    try {
      ...
      Alert.alert('Error', 'Could not fetch download options for current track.');
  ...
  const handleSelectDownloadOption = (option: DownloadOption) => {
    ...
    // For next step, this will be:
    // downloadCtrl.startDownload(playback.currentTrack, option, mediaInfoForDownloadOpts);
    Alert.alert('Download Queued (Conceptual)', `Track: ${playback.currentTrack.title}\nFormat: ${option.format}\nQuality: ${option.quality}\n\n(Full download via DownloadContext in next step)`);
    // setMediaInfoForDownloadOpts(null); // Keep it cached if user reopens modal for same track
  };
  ...
  const styles = StyleSheet.create({
    ...
    contentContainer: { flexGrow: 1, paddingHorizontal: theme.spacing.lg, paddingTop: theme.spacing.sm, paddingBottom: theme.spacing.xl, justifyContent: 'space-between' }, // Adjusted padding & justify
    ...
    actionButtonSmall: { padding: theme.spacing.sm, }
  });
  ...
  if (playback.error && !playback.isLoading && !playback.currentTrack) { // Only show full screen error if no track is loaded at all
     return <View style={styles.centered}><Text style={styles.errorText}>Playback Error: {playback.error}</Text></View>;
  }
  if (!playback.currentTrack) {
    return <View style={styles.centered}><Text style={styles.emptyText}>Nothing selected to play.</Text></View>;
  }

  const currentTrack = playback.currentTrack; // Safe to use now
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.trackInfoContainer}>
        ...
        {playback.error && <Text style={[styles.errorText, {fontSize: theme.typography.fontSize.caption}]}>Error: {playback.error}</Text>}
      </View>
      ...
      <View style={styles.controlsContainer}>
        ...
        <TouchableOpacity onPress={playback.togglePlayPause} style={[styles.controlButton, styles.playPauseButton, (playback.isLoading || !currentTrack || trackDuration === 0 || !!playback.error) && {backgroundColor: theme.colors.textMuted}]} disabled={playback.isLoading || !currentTrack || trackDuration === 0 || !!playback.error}>
          {playback.isLoading ? <ActivityIndicator size='large' color={theme.colors.white} /> : <Icon name={playback.isPlaying ? 'Pause' : 'Play'} size={40} color={theme.colors.white} />}
        </TouchableOpacity>
      ...
      <View style={styles.actionsRow}>
        ...
        <TouchableOpacity style={styles.actionButtonSmall} onPress={openDownloadOptionsForCurrentTrack} disabled={!currentTrack || isLoadingDownloadOptions}>
            {isLoadingDownloadOptions ? <ActivityIndicator size="small" color={theme.colors.accentPrimary}/> : <Icon name="Download" size={24} color={theme.colors.textSecondary} /> }
        </TouchableOpacity>
